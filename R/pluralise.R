...
#'
#' This function takes a given value and outputs whether a selected word needs to be pluralised based on the value of the input:
#' if the value is 1, the output is not pluralised, if the value is > 1, the output is pluralised.
#' The function can either pluralise a specific input word, or the output can be used next to an existing word (if the word parameter is left blank).
#' NB - this function simply adds an 's' to pluralise; for more complex pluralisations (e.g. 'ies' rather than 'ys') this function will not provide desired output.
#'
...
#'
#' @examples
#' pluralise(4, "percentage point") # returns "percentage points"
#' pluralise(1, "week") # returns "week"
#' pluralise(2) # returns "s"
#' pluralise(1) # returns ""
#' pluralise(-5, "week") # returns "weeks"
#'
#' @export
